...
<body>
  ...
  <table width="100%">
    ...
    <?php
    $tgl = date('Y-m-d');
    $detail = "SELECT * FROM orderan WHERE tanggal = '".$tgl."' ";
    ...
    <tfoot>
        <tr>
            ...
            <?php
            $nun = "SELECT * FROM orderan WHERE status_order = 'dikonfirmasi' AND tanggal = '".$tgl."' ";
        ...
        <tr>
            ...
            <td align="right" class="gray">
            <?php
            $nun = "SELECT * FROM orderan WHERE tanggal = '".$tgl."' ";
            $nunres = mysqli_query($dbh,$nun);
            while($s = mysqli_fetch_array($nunres)){
             $sdazz2 = "SELECT * FROM detail_order WHERE id_order = $s[0]";
             $qesz2 = mysqli_query($dbh,$sdazz2);
...
$dompdf->render();
$tgl = date('Y-m-d');
$dompdf->stream("DataSemuaPenjualan ".$tgl,array("Attachment"=>1));
?>